        ...
        for rho_value in config.rho_grid:
            ...
            for method_name in config.spcauchy_methods:
                ...
                record = _benchmark_one_runtime_config(
                    f"spcauchy_{'auto' if method_name == 'auto' else method_name}",
...
def plot_robustness_results(records: list[dict], layout: OutputLayout) -> list[str]:
    ...
    if sp_records:
        methods = sorted({record["method"] for record in sp_records})
    ...
    vmf_records = [record for record in records if record["family"] == "vmf"]
    if vmf_records:
        methods = sorted({record["method"] for record in vmf_records})
