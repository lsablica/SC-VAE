"""Experiment-local robust vMF implementation.

This copy isolates the smallNORB repairs from the existing MNIST and runtime
benchmark implementation in ``benchmark/vendor_vmf_robust``. It uses the
algebraically exact smooth rationalization of the rejection parameter and a
D=33 half-integer recurrence for the experiment's high-kappa range.
"""
...
#   - log I_v(z) via a low-kappa 0F1 series
#   - exact scaled half-integer recurrence at D=33 and high kappa
# ============================================================
...
def log_besselI_via_0f1(nu: Number, kappa: torch.Tensor,
    ...
    """
    Compute log I_nu(kappa) using a low-kappa positive series and, for the
    D=33 order used by smallNORB, an exact high-kappa recurrence.
    ...
    # The smallNORB ambient dimension D=33 gives nu=15.5. For kappa >= 50,
    # use the exact half-integer recurrence on exp(-kappa) I_nu(kappa).
    # This is differentiable, constant cost, and avoids a series whose safe
    # length would otherwise grow linearly with kappa.
    if float(nu) == 15.5:
        large_k = k.clamp_min(50.0)
        common = 0.5 * torch.sqrt(2.0 / (math.pi * large_k))
        exponential = torch.exp(-2.0 * large_k)
        previous = common * (1.0 + exponential)  # I_{-1/2} exp(-k)
        current = common * (-torch.expm1(-2.0 * large_k))  # I_{1/2}
        order = 0.5
        for _ in range(15):
            following = previous - (2.0 * order / large_k) * current
            previous, current = current, following
            order += 1.0
        large_result = large_k + torch.log(current)

        small_k = k.clamp_max(50.0)
        small_x = small_k.square() / 4.0
        small_b = torch.full_like(small_k, b)
        small_log0f1 = log0f1_series_parallel(
            small_b, small_x, K=128
        )
        small_result = (
            nu_t * torch.log(small_k / 2.0)
            - torch.lgamma(nu_t + 1.0)
            + small_log0f1
        )
        return torch.where(k >= 50.0, large_result, small_result)
...
class VonMisesFisher(torch.distributions.Distribution):
    ...
    SciPy Bessel calls replaced by torch-only:
      ...
      - log I_{m/2-1}(kappa) via the stable routine above
    """
    arg_constraints = {
    ...
    def __sample_w_rej(self, shape):
        ...
        # Rationalizing (-2*kappa + c) / (m - 1) gives this exact form.
        # It avoids catastrophic cancellation at large kappa without the
        # old piecewise approximation over [10, 11], whose interpolation
        # derivative creates a concentration optimization barrier.
        b = (self.__m - 1) / (2.0 * self.scale + c)
