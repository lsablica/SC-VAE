...
from experiments.mnist.models import create_model  # noqa: E402
...
def _history_fieldnames() -> list[str]:
    return [
        ...
        "eval_recon_loss",
        "eval_kl",
        "eval_concentration_mean",
        "eval_concentration_std",
        "eval_concentration_q95",
        "eval_concentration_max",
        "eval_fraction_above_0_9",
        "learning_rate",
    ]


def train_run(
    run_spec, data_dir: Path, output_root: Path, device: str, force: bool = False
) -> dict:
    ...
    try:
        for epoch in range(1, run_spec.epochs + 1):
            ...
            eval_metrics = evaluate_model(
                ...
                eval_loader,
                device,
                desc=f"Epoch {epoch}/{run_spec.epochs} [Eval]",
            )
            ...
            row = {
                ...
                "eval_concentration_mean": (
                    eval_metrics["concentration"]["mean"]
                    if eval_metrics["concentration"] is not None
                    else None
                ),
                "eval_concentration_std": (
                    eval_metrics["concentration"]["std"]
                    if eval_metrics["concentration"] is not None
                    else None
                ),
                "eval_concentration_q95": (
                    eval_metrics["concentration"]["q95"]
                    if eval_metrics["concentration"] is not None
                    else None
                ),
                "eval_concentration_max": (
                    eval_metrics["concentration"]["max"]
                    if eval_metrics["concentration"] is not None
                    else None
                ),
                "eval_fraction_above_0_9": (
                    eval_metrics["concentration"]["fraction_above_0_9"]
                    if eval_metrics["concentration"] is not None
                    else None
                ),
            ...
            if eval_metrics["recon_loss"] < best_eval_recon:
                ...
                save_checkpoint(
                    checkpoint_path,
                    checkpoint_payload(model, optimizer, scheduler, epoch, run_spec),
                )
                best_summary = {
                    ...
                    "selected_concentration": eval_metrics["concentration"],
    ...
    save_checkpoint(
        ...
        checkpoint_payload(model, optimizer, scheduler, run_spec.epochs, run_spec),
    )
    wall_clock_training_s = time.perf_counter() - training_started
    if best_summary is not None:
        best_summary["wall_clock_training_s"] = wall_clock_training_s
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train reproducible MNIST benchmark runs."
    )
